...
## v3.1.1
- **Fix: Dauereinspeisung an Kalibriertagen (HMS drosseln nicht)**: An jedem Kalibrier-Fälligkeitstag hob der Controller die interne Regel-Obergrenze `soc_max_limit` tagsüber auf 100 %, obwohl der eigentliche Kalibrier-*Modus* erst nach Sonnenuntergang startet. Ein Speicher, der physisch bei ~95 % dicht ist (Live-Befund 27.07.: L1 in Reparatur/deaktiviert, L2 einziger Speicher, hängt bei 95 %), erschien dem Tag-Regler dadurch als „noch nicht voll" → er öffnete die Hoymiles hart auf 3600 W „zum Laden" und ließ IS/GS voll laufen. Da der Akku nichts aufnahm, floss der gesamte Solarüberschuss (~1,6 kW) den ganzen Tag ins Netz — bei ausgeschaltetem Volleinspeise-/Bypass-Modus. Die Regel-Obergrenze ist jetzt von der Kalibrier-Ziel-Ladegrenze entkoppelt: der Tag-/Nacht-/Bypass-Regler nutzt immer `soc_normal_max` (95 %) und drosselt bei Überschuss korrekt die Hoymiles. Der nächtliche Kalibrier-Modus (dedizierter Zweig, `GS=-2400`/`SA=100`) lädt weiterhin auf 100 %; der solare Tages-Vorlauf auf 100 % entfällt bewusst zugunsten garantierter Nulleinspeisung.
